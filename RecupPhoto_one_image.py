...
url = "http://192.168.243.84:8080//shot.jpg"
...
while True:
    ...
    if cv.waitKey(1) == 27:
        cv.imwrite('C:\Espace_Python\mon espace\TAI\TP2_CV\PROJET\Voitures\output.jpg', img)
...
mat = [cv.imread(file2) for file2 in glob.glob("queries\*.png")]
licence = np.array(licence)
mat = np.array(mat)
print(len(licence))
for i in range(len(licence)):
    Sift_detector(mat,licence[i])